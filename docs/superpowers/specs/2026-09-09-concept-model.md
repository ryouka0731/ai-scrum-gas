...
以下の5件は、モデルと実装の食い違いを**記録として残す**ものである。**いずれも画面は
...
### 5. `--ink-3` は三次情報の段として機能していない

`--ink-2` との比は現在ライトで **1.183**（旧 1.862）。コントラストの下限（`--card` /
`--bg` 上で 4.5:1）を満たそうとすると `--ink-2` に近づかざるを得ないため、
**色だけで3段目を作ることは原理的に成立しない**（総当たりで確認した上限は 1.268）。

三次情報（`.count` の件数・`.card .dot`・`.empty`・`.field .hint` 等）をさらに弱めたい
ときは、**色ではなく文字サイズや区切り線で段を作ること。** `--ink-3` は次の段階で
廃止し `--ink-2` に統合する予定。
