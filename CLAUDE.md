...
- **可視化・操作**: `gas/` の Apps Script が Drive の `scrum/` を読み、共有スプレッドシート（DB）を
  再構築する。人は PBI の状態変更を `gas/web_app.js` が提供する **Web アプリ**（カンバン画面）
  から行う。ベロシティ・バーンダウン等それ以外の情報は引き続きスプレッドシートを見て確認する
  （第1段階の Web アプリはカンバンのみ対応。スプレッドシート側は編集しない ―
  次回同期で失われる仕様のため。今後の段階で Web アプリへ移していく予定）

人がやること・エージェント（あなた）がやることの分担は次の通り。

- ローカルの Claude Code（あなた）は従来どおり `scrum/` のファイルを直接読み書きする。ここは変わらない
- 人は Web アプリのカンバンから PBI の状態（New/Ready/In Progress/Review/Done）を変える。この操作は
  `scrum/product_backlog.csv` へ即座に書き戻される。**Web アプリが読み書きするのはこの1ファイルだけ**で、
  共有スプレッドシートには触れない
