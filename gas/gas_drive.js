...
function getScrumFolder_() {
  const id = getFolderId_();
  if (!id) {
    throw configError_(
  ...
  try {
    ...
    throw configError_('フォルダ ID ' + id + ' にアクセスできません。ID と共有設定を確認してください。');
  }
  const it = root.getFoldersByName('scrum');
  if (!it.hasNext()) {
    throw configError_('指定フォルダの直下に scrum フォルダがありません。');
  }
  return it.next();
}

/** フォルダ直下のファイル本文を返す。無ければ null。 */
function readTextFile_(folder, name) {
...
function findLatestSprintFolder_(scrumFolder) {
  const names = [];
  const byName = {};
