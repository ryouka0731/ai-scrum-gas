...
// Web アプリとしてのデプロイに失敗しても、スプレッドシート自体は既に使える状態のため
// ここで止めない。管理者が Apps Script の画面から手動でデプロイし直せる。
console.log('==> Web アプリとしてデプロイします');
const deployed = runClasp(['create-deployment', '-d', 'AI Scrum ボード'], {
  cwd: GAS_DIR,
  allowFailure: true,
});
if (!deployed) {
  console.log(
    '（Web アプリのデプロイに失敗しました。Apps Script の画面を開き、左メニュー「デプロイ」→\n' +
    '　「新しいデプロイ」→ 種類の歯車アイコンから「ウェブアプリ」を選んで手動でデプロイしてください）'
  );
}
...
作成しました。続けて次を行ってください。
  ...
  4. 「今すぐ同期」を実行して9枚のシートができることを確認する
     （初回は Drive の読み書きを求める承認ダイアログが出ます）
  5. 「自動同期を有効にする（30分毎）」を実行する
  6. Web アプリの URL をメンバーに共有する。URL は Apps Script の画面を開き、
     左メニュー「デプロイ」→「デプロイを管理」で確認できる
     （上のデプロイが失敗していた場合は、ここから手動でデプロイしてから確認する）
